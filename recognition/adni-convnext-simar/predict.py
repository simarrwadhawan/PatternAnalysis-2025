"""
predict.py - Comprehensive Model Evaluation and Visualization

This script:
1. Loads a trained ConvNeXt model
2. Evaluates on the test set
3. Generates all visualizations for the README
4. Computes and saves all metrics

Usage:
    python predict.py --checkpoint runs/adni_convnext/best.pt
"""
...
def evaluate_model(model, loader, device):
    ...
    with torch.no_grad():
        for x, y, paths in tqdm(loader, desc="Evaluating"):
            ...
            # FIX: Convert tensor labels to integers
            all_labels.extend([label.item() if torch.is_tensor(label) else label for label in y])
...
def plot_roc_curve(y_true, y_probs, save_path):
    ...
    # Get probabilities for positive class (AD = 0)
...
def plot_metrics_comparison(metrics, save_path):
    ...
    bars = plt.bar(metric_names, metric_values, color='steelblue', alpha=0.8, edgecolor='black')
...
def plot_sample_predictions(dataset, predictions, labels, probs, save_path, num_samples=16):
    ...
    for idx, ax in enumerate(axes):
        ...
        # FIX: Convert tensor label to int
...
def plot_misclassified(dataset, predictions, labels, probs, save_path, num_samples=16):
    ...
    indices = np.random.choice(misclassified_idx, num_samples, replace=False)
...
def main():
    ...
    plot_combined_confusion_roc(labels, predictions, probs, 
                                save_dir / "confusion_matrix_roc.png")
    
    plot_metrics_comparison(metrics, save_dir / "performance_metrics.png")
    
    plot_sample_predictions(test_set, predictions, labels, probs, 
                          save_dir / "sample_predictions.png", num_samples=16)
    
    plot_misclassified(test_set, predictions, labels, probs,
                      save_dir / "misclassified_samples.png", num_samples=16)
    ...
    print("  • metrics.json")
    print("\nTo download images to your local machine, run:")
    print(f"  scp -r s4977354@rangpur.rcc.uq.edu.au:{save_dir.absolute()} ./")
    print("\n" + "="*70)
